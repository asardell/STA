...
| **Somme des carrés entre les groupes (SSB)** | Mesure de la variabilité entre les groupes            | SSB       | $\text{SSB} = n \sum (\bar{X}_i - \bar{X})^2$ où $\bar{X}_i$ est la moyenne du groupe $i$ |
| **Somme des carrés à l'intérieur des groupes (SSW)** | Mesure de la variabilité à l'intérieur des groupes     | SSW       | $\text{SSW} = \sum (X_{ij} - \bar{X}_i)^2$ où $\bar{X}_i$ est la moyenne du groupe $i$
i-ème groupe.  |
